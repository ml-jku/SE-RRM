...
def aug(name: str, trans_id: int):
    # Augment plan
    #trans_id = np.random.randint(0, 8)
    mapping = np.concatenate([np.arange(0, 1, dtype=np.uint8), np.arange(1, 10, dtype=np.uint8)])  # Permute colors, Excluding "0" (black)
    
    name_with_aug_repr = f"{name}{PuzzleIdSeparator}t{trans_id}{PuzzleIdSeparator}{''.join(str(x) for x in mapping)}"

    def _map_grid(grid: np.ndarray):
        return dihedral_transform(grid, trans_id)
...
def convert_single_arc_puzzle(results: dict, name: str, puzzle: dict, aug_bool: bool, dest_mapping: Dict[str, Tuple[str, str]]):
    # Convert
    ...
    if aug_bool:
        ...
        for _trial in range(8):
            aug_name, _map_grid = aug(name, _trial)
...
def load_puzzles_arcagi(config: DataProcessConfig):
    ...
    for subset_name in config.subsets:
        ...
        for idx, (name, puzzle) in enumerate(puzzles):
            ...
            convert_single_arc_puzzle(results, name, puzzle, config.aug, {"train": train_examples_dest, "test": test_examples_dest})
            total_puzzles += 1
